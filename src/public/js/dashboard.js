...
export function updateYuukaSpeechBubble() {
  ...
  if (state.totalExpensesVal > 30000) {
    ...
    el.textContent = "お疲れ様です、先生。セミナー会計の早瀬ユウカが、今日も完璧にサポートしますよ！タスクや予定、家計の管理なら私に何でもお任せください！";
  }
}
...
export async function fetchDashboardStats() {
  try {
    ...
    document.getElementById("stat-expenses-total").textContent     = `¥${state.totalExpensesVal.toLocaleString()}`;
    ...
    document.getElementById("dashboard-highest-category").textContent = expenseData.breakdown?.[0]?.category || "なし";
    document.getElementById("dashboard-average-expense").textContent  = expenses.length > 0 ? `¥${Math.round(expenseData.total / 30).toLocaleString()}` : "¥0";

    renderPriceTrendChart(
      expenses,
      p => {
        const label = p.x === 400 ? "今日" : p.x === 320 ? "昨日" : "この日";
        document.getElementById("yuuka-bubble-text").textContent = `${label}の出費額は ¥${p.amount.toLocaleString()} ですよ、先生！`;
      },
      updateYuukaSpeechBubble
    );
  } catch (err) {
    console.error("ダッシュボード情報の更新エラー:", err);
    document.getElementById("yuuka-bubble-text").textContent = "先生、ダッシュボード情報の取得中にエラーが発生しました。データベース接続を確認してください！";
  }
}
...
function renderPriorityBarChart(priorities) {
  ...
  const colors  = isLight
    ...
    : ["#71717a", "#e4e4e7", "#fafafa"];
  const labels   = ["低", "中", "高"];
  const maxCount = Math.max(priorities[0], priorities[1], priorities[2], 1);

  [0, 1, 2].forEach(p => {
    const wrap = document.createElement("div");
    wrap.style.cssText = "display:flex;flex-direction:column;align-items:center;flex:1;height:100%;justify-content:flex-end;";
    wrap.title = `${labels[p]}優先度: ${priorities[p]}件`;

    const countEl = document.createElement("span");
    countEl.style.cssText = "font-size:0.55rem;font-family:var(--font-family-mono);color:var(--color-zinc-muted);margin-bottom:2px;";
...
function renderSparkline(id, trend, minScale) {
  ...
  const pts    = trend.map((v, i) => `${i === 0 ? "" : "L"} ${i * 25},${19 - (v / maxVal) * 18}`).join(" ");
